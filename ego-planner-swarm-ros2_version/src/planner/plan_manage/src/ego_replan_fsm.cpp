...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    node_->declare_parameter("fsm/flight_type", 1);  // 默认为MANUAL_TARGET
    ...
    // 确保target_type_是有效值
    if (target_type_ <= 0 || target_type_ > 3)
    {
      RCLCPP_WARN(node_->get_logger(), "Invalid target_type_ %d, setting to MANUAL_TARGET", target_type_);
      target_type_ = MANUAL_TARGET;
    }
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    if (success)
    {
      ...
      int i_end = std::max(1, static_cast<int>(floor(duration / step_size_t)));
      
      // 限制最大点数
      constexpr int max_points = 1000;
      if (i_end > max_points)
      {
        RCLCPP_WARN(node_->get_logger(), "Limiting trajectory points from %d to %d", i_end, max_points);
        i_end = max_points;
      }
      ...
      {
        RCLCPP_ERROR(node_->get_logger(), "Error generating trajectory: %s", e.what());
      }
    }
    else
    {
  ...
  void EGOReplanFSM::waypointCallback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg)
  {
    ...
    if (!have_odom_)
    {
      RCLCPP_WARN(node_->get_logger(), "No odometry data yet, ignoring waypoint");
      return;
    }
